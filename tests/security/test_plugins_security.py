...
from husarz.plugins.client import _validate_mcp_endpoint
...
def test_loopback_allowed(endpoint: str) -> None:
    _validate_mcp_endpoint(endpoint, _DENY)  # nie rzuca
...
def test_internal_and_metadata_hard_blocked(endpoint: str) -> None:
    with pytest.raises(EgressError):
        _validate_mcp_endpoint(endpoint, _DENY)
...
def test_public_http_rejected_requires_https() -> None:
    with pytest.raises(PluginError):
        _validate_mcp_endpoint("http://mcp.example.com", _DENY)


def test_public_https_without_allowlist_denied() -> None:
    with pytest.raises(EgressError):
        _validate_mcp_endpoint(
            "https://mcp.example.com", _DENY, resolve=_resolves_to("93.184.216.34")
        )
...
def test_public_https_allowlisted_ok() -> None:
    ...
    _validate_mcp_endpoint("https://mcp.example.com", egress, resolve=_resolves_to("93.184.216.34"))


def test_userinfo_rejected() -> None:
    with pytest.raises(PluginError):
        _validate_mcp_endpoint("http://user:pass@127.0.0.1:8808", _DENY)
...
def test_domain_resolving_to_metadata_blocked() -> None:
    ...
    with pytest.raises(EgressError):
        _validate_mcp_endpoint(
            "https://mcp.vendor.com", egress, resolve=_resolves_to("169.254.169.254")
        )
...
def test_domain_resolving_to_private_blocked_under_allow_policy() -> None:
    egress = EgressConfig(default_policy=EgressPolicy.ALLOW)
    with pytest.raises(EgressError):
        _validate_mcp_endpoint("https://x.attacker.net", egress, resolve=_resolves_to("10.0.0.9"))


def test_unresolvable_domain_fails_closed() -> None:
    egress = EgressConfig(allowlist=["vendor.com"])
    with pytest.raises(EgressError):
        _validate_mcp_endpoint("https://mcp.vendor.com", egress, resolve=_resolves_to())
